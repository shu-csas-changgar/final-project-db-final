...
DELIMITER $$
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`owner_BEFORE_INSERT` BEFORE INSERT ON `owner` FOR EACH ROW
BEGIN
IF (owner.employee_id IS NULL AND owner.location_id IS NULL) THEN
	SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = '\'employee_id\' and \'location_id\' cannot both be null';
  END IF;
END$$
